...
that ones adminvob, if one exists.
...
trigger by the event mkactivity, only in the setactivity event we can gain access to the environment variable CLEARCASE_ACTIIVITY.
...
Please refer to the description at the project homepage, see below for link
