...
use constant ACC_TEMP_LBTYPE_NAME        => '^_\._\d+_\._\d+$'                 ;    # -c "RegExp defining temp lbtype name while applying Restricted labels with the applyrestricted.pl utility script."
...
=cut
   ...
   system("cleartool desc $full_lb >nul"); $? && exit 1;
   system("cleartool desc $full_br >nul"); $? && exit 1;
   system("cleartool desc hltype:".acc::HLTYPE_RESTRICTED."\@$$vobref >nul"); $? && exit 1;
   ...
   if ( (lc($ENV{acc::CLEARCASE_FORCE_RESTRICTION}) ne "true") && get_to_hlinks(\$full_lb, \"GlobalDefinition", \@_)) {
     ...
     print STDERR "You should manually run:\n".
                  ...
                  "Define a system variabel named ".acc::CLEARCASE_FORCE_RESTRICTION." (value should be \"true\")\n".
...
=cut
  ...
  my %types = (
    ...
    "attype:".acc::ATTYPE_PROMOTION_LEVEL."\@".$vob => "-c \"ACC meta type\" -vtype string -enum \\\"released\\\",\\\"tested\\\",\\\"built\\\",\\\"integrated\\\" ",
