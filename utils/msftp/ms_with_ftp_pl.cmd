...
sub get_sclass_for_incoming {
	...
	( $log->error( "Failed to get vobtag at line " . __LINE__ ) && return ) unless ($tag);
...
sub init_psftp_host {
	...
	unless ( exists $psftp_known_hosts{ $parms{server} } ) {
		...
		open( PS, "| $cmd" ) || $log->error("Command $cmd Failed: $!\n");
		print PS "y\n";
		print PS "exit\n";
		close(PS);
...
sub get_from_sftp {
	...
	$log->error("Details for $inisection not found in ini file") unless exists $inicontents{$inisection};
	my $s_user = ${ $inicontents{$inisection} }{user};
	$log->error("User for replication with $inisection not found in ini file") unless ($s_user);
	my $s_pass = ${ $inicontents{$inisection} }{password};
	$log->error("User password for replication with $inisection not found in ini file") unless ($s_pass);
	...
	foreach (@remotefiles) {
		if (/^Unable to/i) {
			$log->error( "Unexpected psftp reply:\n" . join( '', @remotefiles ) );
		}
		$log->information("$_");
		next unless /^-/;
...
sub get_from_ftp {
	...
	( $log->error("Cant find Storage bay for class $sclass") && return ) unless exists $storageclasses{$sclass};
	...
	( $log->error( "Cannot change directory $serverpath " . $ftpObject->message ) && return ) unless $ftpObject->cwd("$serverpath");
...
sub start_ftp {
	...
	# connect to ftp
	( $log->error("Cannot connect to $parms{server}: $@") && return ) unless $ftpObject = Net::FTP->new( $parms{server}, Debug => 0 );
	( $log->error( "Cannot log in: " . $ftpObject->message ) && return ) unless $ftpObject->login( $parms{user}, $parms{password} );
	$ftpObject->binary();
}
